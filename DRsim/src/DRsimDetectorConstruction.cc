...
G4VPhysicalVolume* DRsimDetectorConstruction::Construct() {
  ...
  fFrontL     = 1000.;     // NOTE :: Length from the center of world box to center of module
  fTowerDepth = 320.; 
  fModuleH    = 30;
  fModuleW    = 30;
  fFiberUnitH = 1.;
  ...
  ModuleBuild(ModuleLogical,PMTGLogical,PMTfilterLogical,PMTcellLogical,PMTcathLogical,fiberUnitIntersection,fiberCladIntersection,fiberCoreIntersection,fModuleProp);
...
void DRsimDetectorConstruction::ConstructSDandField() {
  ...
  if ( doPMT ) {
    for (int i = 0; i < fNofModules; i++) {
      DRsimSiPMSD* SiPMSDmodule_left = new DRsimSiPMSD("Module"+std::to_string(2*i), "ModuleC"+std::to_string(2*i), 0, fModuleProp.at(i));
      DRsimSiPMSD* SiPMSDmodule_right = new DRsimSiPMSD("Module"+std::to_string(2*i+1), "ModuleC"+std::to_string(2*i+1), 1, fModuleProp.at(i));
...
void DRsimDetectorConstruction::ModuleBuild(G4LogicalVolume* ModuleLogical_[], 
  ...
  for (int i = 0; i < fNofModules; i++) {    
    ...
    if ( doPMT ) {
      ...
      PMTGLogical_[2*i]    = new G4LogicalVolume(pmtg,FindMaterial("G4_AIR"),moduleName);
      PMTGLogical_[2*i+1]  = new G4LogicalVolume(pmtg,FindMaterial("G4_AIR"),moduleName);
      new G4PVPlacement(0,dimCalc->GetOrigin_PMTG(2*i),PMTGLogical_[2*i],moduleName,worldLogical,false,0,checkOverlaps);
      new G4PVPlacement(0,dimCalc->GetOrigin_PMTG(2*i+1),PMTGLogical_[2*i+1],moduleName,worldLogical,false,0,checkOverlaps);
    }
    ...
    if ( doPMT ) {
      ...
      new G4PVPlacement(0,G4ThreeVector(0.,0.,0.),SiPMlayerLogical_left,"SiPMlayerPhysical",PMTGLogical_[2*i],false,0,checkOverlaps);
      new G4PVPlacement(0,G4ThreeVector(0.,0.,0.),SiPMlayerLogical_right,"SiPMlayerPhysical",PMTGLogical_[2*i+1],false,0,checkOverlaps);

      G4VSolid* PMTcellSolid = new G4Box("PMTcellSolid", 1.1/2. *mm, 1.1/2. *mm, PMTT/2. *mm );
      PMTcellLogical_[2*i] = new G4LogicalVolume(PMTcellSolid,FindMaterial("Glass"),"PMTcellLogical_");
      PMTcellLogical_[2*i+1] = new G4LogicalVolume(PMTcellSolid,FindMaterial("Glass"),"PMTcellLogical_");

      DRsimCellParameterisation* PMTcellParam = new DRsimCellParameterisation(fTowerXY.first,fTowerXY.second,fModuleH,fModuleW);
      G4PVParameterised* PMTcellPhysical_left = new G4PVParameterised("PMTcellPhysical_left",PMTcellLogical_[2*i],SiPMlayerLogical_left,kXAxis,fTowerXY.first*fTowerXY.second-fTowerXY.first/2,PMTcellParam);
      G4PVParameterised* PMTcellPhysical_right = new G4PVParameterised("PMTcellPhysical_right",PMTcellLogical_[2*i+1],SiPMlayerLogical_right,kXAxis,fTowerXY.first*fTowerXY.second-fTowerXY.first/2,PMTcellParam);

      G4VSolid* PMTcathSolid = new G4Box("PMTcathSolid", 1.1/2. *mm, 1.1/2. *mm, PMTT/2. *mm );
      PMTcathLogical_[2*i] = new G4LogicalVolume(PMTcathSolid,FindMaterial("Silicon"),"PMTcathLogical_");
      PMTcathLogical_[2*i+1] = new G4LogicalVolume(PMTcathSolid,FindMaterial("Silicon"),"PMTcathLogical_");
      new G4PVPlacement(0,G4ThreeVector(0.,0.,0.),PMTcathLogical_[2*i],"PMTcathPhysical",PMTcellLogical_[2*i],false,0,checkOverlaps);
      new G4PVPlacement(0,G4ThreeVector(0.,0.,0.),PMTcathLogical_[2*i+1],"PMTcathPhysical",PMTcellLogical_[2*i+1],false,0,checkOverlaps);
      new G4LogicalSkinSurface("Photocath_surf_left",PMTcathLogical_[2*i],FindSurface("SiPMSurf"));
      new G4LogicalSkinSurface("Photocath_surf_right",PMTcathLogical_[2*i+1],FindSurface("SiPMSurf"));
...
void DRsimDetectorConstruction::FiberImplement(G4int i, G4LogicalVolume* ModuleLogical__[], 
  ...
  int NofPlate = fModuleH/(1.22); 
  int NofFiber = fModuleW/(1.35);    
  ...
  for (int k = 0; k < NofPlate; k++) {
    for (int j = 0; j < NofFiber; j++) { 
      ...
      */
      ...
      G4float fX = -fModuleH*mm/2 + k*1.22*mm + 0.61*mm;
      G4float fY = -fModuleW*mm/2 + j*1.35*mm + 0.675*mm;
      if (fWhich) fY += 0.675*mm;
